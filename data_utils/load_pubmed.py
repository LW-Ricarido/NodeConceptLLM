...
def parse_pubmed():
    path = '{}/data/'.format(local_raw_dataset_dir)
...
def get_raw_text_pubmed(use_text=False, seed=0):
    ...
    if not use_text:
        return to_dgl(data), None
    
    f = open('{}/pubmed.json'.format(local_raw_dataset_dir))
    pubmed = json.load(f)
    ...
    for ti, ab in zip(TI, AB):
        t = 'Title: ' + ti + '\n'+'Abstract: ' + ab
        text.append(t)
    return to_dgl(data), text

def convert_text_into_embeddings(device):
    graph, text = get_raw_text_pubmed(use_text=True)
    PLM = SentenceTransformer('all-mpnet-base-v2').to(device)
    embedding_to_text = []
    raw_feature = torch.zeros((graph.num_nodes(), 768),device=device)
    import ipdb; ipdb.set_trace()
    for i in range(graph.num_nodes()):
        if 'Diabetes mellitus type 2' in text[i] or 'diabetes mellitus type 2' in text[i]:
            print(graph.ndata['y'][i])
            import ipdb;ipdb.set_trace()
        if i == 106:
            print('106 is here.')
            import ipdb; ipdb.set_trace()
        raw_feature[i] = PLM.encode(text[i],convert_to_tensor=True)
        embedding_to_text.append({'input_embedding':raw_feature[i].cpu(), 'label':text[i]})
    graph.ndata['raw'] = raw_feature.cpu()
    # torch.save(embedding_to_text,'datasets_local/pubmed_embedding_to_text.pt')
    # dgl.save_graphs('pubmed_graph_withPLMfeature.bin',[graph])

if __name__ == "__main__":
    device = torch.device('cuda:0')
    convert_text_into_embeddings(device)