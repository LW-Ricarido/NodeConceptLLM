...
from trl import GRPOTrainer, maybe_apply_chat_template
from trl.trainer.grpo_trainer import nanstd, truncate_with_protected_tokens
...
class MM_GRPOTrainer(GRPOTrainer):
    
    def __init__(self,registered_input_keys:List[str]=None,*args, **kwargs):
        super().__init__(*args,**kwargs)
        self.registered_input_keys = registered_input_keys
    ...
    def _generate_and_score_completions(
        ...
        for key in self.registered_input_keys:
            if key not in prompt_inputs.keys():
                prompt_inputs[key] = [torch.tensor(x[key]) for x in inputs]
